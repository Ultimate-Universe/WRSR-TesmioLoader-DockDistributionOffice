...
// Dock Distribution Office v1.1.0 for WRSR v1.1.1.9.
// TesmioLoader plugin API v4.
//
// The native Distribution Office panel stores player tasks. A mod-owned
// controller evaluates those tasks, reserves destination jobs, and creates
// native ship schedules. WRSR continues to handle pathfinding, movement,
// docking, cargo transfer, fuel use, and route serialization.
//
// Target: WRSR v1.1.1.9 SOVIET64.exe.
...
#define RVA_BUILDING_DISPATCH 0x00139A70u // FUN_140139a70(game)
#define RVA_SHIP_PANEL        0x007A0C90u // FUN_1407a0c90(ui, window)
#define RVA_ROAD_DO_PANEL     0x00741170u // FUN_140741170(ui, window)
#define RVA_DO_ADD_SELECTOR   0x002B78B0u // FUN_1402b78b0(game, ..., ..., scale)
#define RVA_HIGHLIGHT_BUILDING 0x0040BD60u // FUN_14040bd60(...)
#define RVA_ASSIGN_PUSH       0x0008E050u // vector<assignment*>::push_back
#define RVA_ASSIGN_INIT       0x001512A0u // initialise 0x198 assignment config
#define RVA_QWORD_PUSH        0x000B0E90u // vector<void*>::push_back
#define RVA_CARGO_CAPACITY     0x006BDDB0u // FUN_1406bddb0(vehicle, resource, ...)
#define RVA_CARGO_AMOUNT       0x006BE140u // FUN_1406be140(vehicle, resource, includeChildren)
#define RVA_BUILDING_STATS_BASE  0x001E8350u // FUN_1401e8350(game, outPair, building, resource, mode, scratch)
#define RVA_BUILDING_HAS_EXTRA   0x001E5080u // FUN_1401e5080(game, building, resource, mode)
#define RVA_BUILDING_STATS_EXTRA 0x001E92E0u // FUN_1401e92e0(game, outPair, building, resource, mode, scratch)
#define RVA_ROUTE_TARGET_RESIZE 0x001FEF00u // vector<void*>::resize
#define RVA_ROUTE_CONFIG_RESIZE 0x0044A180u // vector<0x198 config>::resize
#define RVA_ROUTE_CONFIG_INIT   0x0067CD20u // initialise one route config for vehicle
#define RVA_CONFIG_HALF_COPY    0x001E0FC0u // deep-copy one 0xC8 config half
#define RVA_ROUTE_REFRESH       0x006B8EB0u // recalculate native vehicle route target/path
#define RVA_VEHICLE_ROUTE_START 0x00429A80u // activate current native schedule
...
static int AddNativeAssignment(void* home, void* target, const char* label) {
    ...
    *(u64*)(config + 0xE8) = 0;
    ...
    // ship schedules do not require a building back-reference, so their
    // private bookkeeping is intentionally left untouched.
...
static int DrawCompactOverseasButton(void* texture, float x, float y, float icon,
    ...
    // The native ship-schedule buttons draw these two texture objects with
    // the complete 0..1 UV range.  G_PANEL_FULLSIZE_RVA was never part of
    // that path; treating its unrelated runtime float as a UV maximum made
    // the 1.1.1.9 build sample an atlas-sized beige fragment instead of the
    // Soviet/Western icon.
    f_PanelDraw(EXE + G_PANEL_RVA, 0.0f, 0.0f, 1.0f, 1.0f, 0.0f, 1);
...
// --------------------------------------------------------------------- route inspection + custom ship cargo policy
...
static int DestinationReservedByOtherShip(const TrackedBuilding* tracked,
                                          const ActiveShipJob* current,
                                          void* destination) {
    if (!tracked || !destination) return 0;
    for (int i = 0; i < tracked->activeJobCount; ++i) {
        const ActiveShipJob* job = &tracked->activeJobs[i];
        if (current && job->ship == current->ship) continue;
        if (job->destinationTarget == destination) return 1;
    }
    return 0;
}
...
// Native cargo helpers are used only with ships. Building demand is evaluated
// through the verified building-statistics calls below.

// --------------------------------------------------------------------- manager-owned task controller
...
// Replace a completed three-stop temporary schedule while the ship is still
// physically berthed at its old destination.  CURRENT_TARGET deliberately
// remains untouched: it identifies the real berth until the native route
// starter departs.  Only the next route index and future stop/config vectors
// are rewritten, so the ship can start another valid task without first
// visiting its Distribution Dock.
static int RewriteManagedRouteAfterUnload(void* office, ActiveShipJob* job,
                                          const AssignmentTask* source,
                                          const AssignmentTask* destination) {
    if (!office || !job || !job->ship || !source || !destination ||
        !source->resource || source->resource != destination->resource ||
        !f_ConfigHalfCopy || !g_activeGame) return 0;

    void* ship = job->ship;
    int routeCount = PointerVectorCount(ship, V_ROUTE_BEGIN, V_ROUTE_END, 64);
    int configCount = RouteConfigCount(ship);
    if (routeCount != 3 || configCount != 3) return 0;

    void** targets = *(void***)((u8*)ship + V_ROUTE_BEGIN);
    u8* configs = (u8*)ReadPointer(ship, V_ROUTE_CONFIG_BEGIN);
    if (!targets || !configs || !IsReadable(targets, 3 * sizeof(void*)) ||
        !IsReadable(configs, 3 * 0x198u)) return 0;

    float newCapacity = CargoCapacity(ship, destination->resource);
    if (newCapacity <= 0.01f) return 0;

    u8* sourceConfig = configs;
    u8* destinationConfig = configs + 0x198u;
    if (!f_ConfigHalfCopy(sourceConfig, source->config)) return 0;
    if (destination->wildcardResources) {
        if (!f_ConfigHalfCopy(destinationConfig + CFG_UNLOAD_HALF,
                              source->config + CFG_LOAD_HALF)) return 0;
    }
    else if (!f_ConfigHalfCopy(destinationConfig + CFG_UNLOAD_HALF,
                               destination->config + CFG_UNLOAD_HALF)) return 0;

    if (!FilterConfigToResource(sourceConfig, 0, source->resource) ||
        !FilterConfigToResource(destinationConfig, 1, destination->resource)) return 0;
    ...
    *(u64*)(destinationConfig + 400) = *(u64*)(destination->config + 400);
    ...
    int previousCycles = job->directRepeatCount;
    job->resource = destination->resource;
    job->sourceTarget = source->target;
    job->destinationTarget = destination->target;
    job->homeTarget = office;
    job->sourceIndex = 0;
    job->destinationIndex = 1;
    job->homeIndex = 2;
    job->cargoCapacity = newCapacity;
    job->targetLoadFraction = (float)g_minDepartureLoadPermille / 1000.0f;
    job->targetLoadAmount = newCapacity * job->targetLoadFraction;
    job->sourceReserveFraction = source->threshold;
    job->destinationTriggerFraction = destination->threshold;
    job->sourceExternal = source->externalTarget;
    ...
    sourceConfig[CFG_ENABLED] = 1;
    *(float*)(sourceConfig + CFG_THRESHOLD) = safeFraction;
    sourceConfig[0x28u] = 0;
    sourceConfig[CFG_UNLOAD_HALF + CFG_ENABLED] = 0;
    destinationConfig[CFG_ENABLED] = 0;
    destinationConfig[CFG_UNLOAD_HALF + CFG_ENABLED] = 1;
    *(float*)(destinationConfig + CFG_UNLOAD_HALF + CFG_THRESHOLD) = 1.0f;
    destinationConfig[0xF0u] = 0;

    if (!WriteRouteIndex(ship, 0)) return 0;
    if (IsReadable((u8*)ship + V_ROUTE_DIRTY, sizeof(u16)))
        *(u16*)((u8*)ship + V_ROUTE_DIRTY) = 0x0101u;
    if (f_RouteRefresh) f_RouteRefresh((void**)ship, 0, 0, 0);
    if (f_VehicleRouteStart)
        f_VehicleRouteStart((longlong)g_activeGame, (longlong)ship, 1);
    ...
    job->directRepeatCount = previousCycles + 1;
    job->lastRouteIndex = 0;
    job->missingSinceTick = 0;

    if (g_customPolicyDiagnostics && H && H->log) {
        H->log("DockDistributionOffice  manager direct retask armed: ship=%p source=%p destination=%p resource=%p; home berth bypassed cycle=%d",
               ship, source->target, destination->target, destination->resource,
...
// A completed ship must run the same demand/source selection used for idle
// dispatch before accepting its home leg.  TryArmDirectRepeat handles the
// zero-rewrite fast path when the same standing task is still due; this wrapper
// additionally replaces the temporary route when another task is ready.
static int TryArmNextTaskBeforeHome(void* office, ActiveShipJob* job,
                                    u8* sourceConfig, u8* destinationConfig) {
    if (!g_directRepeatEnabled || !office || !job || !job->ship ||
        !g_activeGame) return 0;

    TrackedBuilding* tracked = GetTrackedBuilding(office);
    if (!tracked) return TryArmDirectRepeat(office, job, sourceConfig, destinationConfig);
    ...
    if (!ReadAssignmentTasks(office, sources, MAX_MANAGER_TASKS,
        ...
        return TryArmDirectRepeat(office, job, sourceConfig, destinationConfig);
    }

    for (int d = 0; d < destinationCount; ++d) {
        AssignmentTask* destination = &destinations[d];
        if (DestinationReservedByOtherShip(tracked, job, destination->target)) continue;

        float amount = 0.0f;
        float capacity = 0.0f;
        if (!SafeCargoStats(destination->target, destination->resource,
                            &amount, &capacity) || capacity <= 0.01f) continue;
        if (amount / capacity + g_triggerEpsilon >= destination->threshold) continue;

        AssignmentTask* selectedSource = 0;
        for (int s = 0; s < sourceCount; ++s) {
            if (sources[s].resource != destination->resource) continue;
            if (!SourceCanSupply(&sources[s])) continue;
            selectedSource = &sources[s];
            break;
        }
        if (!selectedSource || CargoCapacity(job->ship, destination->resource) <= 0.01f)
            continue;

        if (selectedSource->target == job->sourceTarget &&
            destination->target == job->destinationTarget &&
            destination->resource == job->resource) {
            return TryArmDirectRepeat(office, job, sourceConfig, destinationConfig);
        }
        if (RewriteManagedRouteAfterUnload(office, job, selectedSource, destination)) {
            return 1;
        }
    }
...
static int UpdateOneActiveJob(void* office, ActiveShipJob* job) {
    ...
    if (destinationActive && onBoard <= 0.01f && job->emptyConfirmed &&
        TryArmNextTaskBeforeHome(office, job, sourceConfig, destinationConfig)) {
        return 1;
    }
    ...
    // Distribution Dock. The route index may advance before CURRENT_TARGET,
    // so clearing it earlier can strand an empty ship at the destination.
...
static int ReconstructAssignedVoyages(TrackedBuilding* tracked, void* office) {
    if (!g_customPolicyEnabled || !tracked || !office ||
        tracked->activeJobCount >= MAX_ACTIVE_JOBS) return 0;
...
static void HookShipPanel(void* ui, void* window) {
    ...
    if (!g_loggedPanelOpen && H && H->log) {
        g_loggedPanelOpen = 1;
        H->log("DockDistributionOffice  opening native Distribution Office panel (correct 2-argument ABI)");
    }

    int assignmentCountBeforePanel = AssignmentCount(building);
    f_RoadDoPanel(ui, window);
...
EXPORT int TsmPluginInit(const TsmHost* host, TsmPluginInfo* info) {
    ...
    g_minDepartureLoadPermille = H->configInt("plugins\\DockDistributionOffice.ini", "DockDistributionOffice", "minimum_departure_load_permille", 999);
    ...
    if (H->log) {
        H->log("DockDistributionOffice  init v1.1.0 release marker=%s default=%s",
...
EXPORT int TsmPluginStart(void) {
    ...
    // supplied v1.1.1.9 decompile at these RVAs. They are not hook sites, so
    // each target must be readable before the controller is enabled.
    if (g_managerEnabled &&
    ...
    if (H->log) {
        ...
        H->log("DockDistributionOffice  building stats calls verified against decompile: base=exe+0x1E8350 linked_test=exe+0x1E5080 linked_extra=exe+0x1E92E0; vehicle cargo helpers retained for ships only");
        ...
        H->log("DockDistributionOffice  manager policy=%d departure=%.1f%% wait_empty=%d source_reserve=enforced reservations=1 reconstruct_world_routes=%d ownership_grace=%d route_dirty_writes=one-shot-only",
               g_customPolicyEnabled, (float)g_minDepartureLoadPermille / 10.0f, g_waitUntilEmpty,
               g_reconstructWorldVehicles, g_ownershipGraceTicks);
